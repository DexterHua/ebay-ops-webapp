...
function normalizeSale(row: SalesData, skuMap: Map<string, SkuData>): SaleMetric | undefined {
  ...
  const firstMile = firstNumber(row as Record<string, unknown>, ["头程费用_USD"], rate > 0 ? (quantity * FIRST_MILE_RMB_PER_ITEM) / rate : 0);
...
function storeMatchesIssue(issue: IssueData, storeName: string, storeId: string, storeLabel: string): boolean {
  const value = text(issue.店铺);
  if (!value) return true;
  return value === storeName || value === storeId || value === storeLabel;
}

function assignSkusToStore(rows: SkuData[], storeId: StoreId): SkuData[] {
  const storeIds = STORES.filter((item) => item.active).map((item) => item.id);
  const storeIndex = storeIds.indexOf(storeId);
  if (storeIndex < 0) return [];
  return rows.filter((_, index) => index % storeIds.length === storeIndex);
}

export default function StorePageClient({ storeId }: { storeId: StoreId }) {
  const id = storeId;
  const store = STORES.find((item) => item.id === id);
  const storeColor = STORE_COLORS[id] || "#475569";
    ...
    Promise.all([
      ...
      .then(([skuResult, summaryResult, strategyResult, issueResult, salesResult, detailResult]) => {
        if (skuResult.success && summaryResult.success && strategyResult.success && detailResult.success) {
          ...
          const inTransitBySku = new Map(summarizeInTransitInventoryBySku(inventoryDetails).map((item) => [item.SKU, item.quantity]));
          const sellableBySku = new Map(summarizeInventoryQuantityByState(inventoryDetails, "橙联可售").map((item) => [item.SKU, item.quantity]));
          const mergedSkus = (skuResult.data || [])
          ...
          setSkus(assignSkusToStore(mergedSkus, store.id));
        } else {
          toast.error("SKU 或库存数据加载失败");
        }

        if (issueResult.success) {
          setIssues((issueResult.data || []).filter((issue: IssueData) => storeMatchesIssue(issue, store.name, store.id, store.label)));
        }
        if (salesResult.success) {
          setSales((salesResult.data || []).filter((row: SalesData) => text(row.店铺) === store.name));
        }
      })
  ...
  const lastYearStart = addDays(currentStart, -365);
  ...
  const inventory = useMemo(() => {
    ...
    const sellable = skus.reduce((sum, sku) => sum + toNumber(sku.橙联可售), 0);
    const inTransit = skus.reduce((sum, sku) => sum + toNumber(sku.橙联在途), 0);
  ...
  const exceptions = useMemo<ExceptionRow[]>(() => {
    ...
    issues
      .filter((issue) => text(issue.优先级) === "高" || text(issue.状态) === "待处理")
      .slice(0, 8)
      .forEach((issue, index) => {
        rows.push({
          id: `issue-${index}-${text(issue.SKU)}`,
          priority: text(issue.优先级) === "高" ? "P1" : "P2",
          sku: text(issue.SKU) || "-",
  ...
  const trendData = useMemo(() => {
    const days = new Map<string, SaleMetric[]>();
    for (let cursor = currentStart; cursor <= periodEnd; cursor = addDays(cursor, 1)) {
      days.set(dayKey(cursor), []);
    }
    currentSales.forEach((sale) => {
      const rows = days.get(sale.day) || [];
      rows.push(sale);
      days.set(sale.day, rows);
    });

    return Array.from(days.entries()).map(([day, rows]) => {
  ...
  const riskBanner = useMemo(() => {
    const p0 = exceptions.filter((row) => row.priority === "P0").length;
    if (p0 > 0) return { tone: "red", title: `${p0} 个高危异常`, desc: "优先处理负利润、缺货和严重库存风险" };
    if (exceptions.length > 0) return { tone: "amber", title: `${exceptions.length} 个经营异常`, desc: "建议按利润、库存、售后顺序逐项闭环" };
    return { tone: "green", title: "经营状态稳定", desc: "当前周期没有高优先级异常" };
  }, [exceptions]);
  ...
  if (!store) {
    return <div className="app-page py-20 text-center text-slate-400">店铺不存在</div>;
  }
    ...
    <div className="app-page">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <div className="mb-3 flex items-center gap-2">
            <Link href="/dashboard" className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-500 shadow-sm transition-colors hover:bg-slate-50" aria-label="返回总览">
              <ArrowLeft className="h-4 w-4" />
            </Link>
            <span className="inline-flex h-8 items-center rounded-lg px-3 text-sm font-semibold text-white" style={{ backgroundColor: storeColor }}>{store.label}</span>
          </div>
          <p className="page-kicker">Store Overview</p>
          <h1 className="page-title">{store.name} 单店经营看板</h1>
          <p className="page-description">
            单店铺 · {periodDays} 天 · 截止 {dayKey(periodEnd)} · SKU {skus.length} · 销售记录 {currentSummary.orders}
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          {PERIODS.map((days) => (
            <Button key={days} type="button" variant={periodDays === days ? "default" : "outline"} size="sm" onClick={() => setPeriodDays(days)}>
              近{days}天
            </Button>
          ))}
          <Button type="button" variant="outline" size="icon-sm" onClick={() => window.location.reload()} aria-label="刷新店铺看板">
            <RefreshCw className="h-3.5 w-3.5" />
          </Button>
        </div>
      </div>

      <div className={`rounded-2xl border px-4 py-3 ${riskBanner.tone === "red" ? "border-red-200 bg-red-50 text-red-800" : riskBanner.tone === "amber" ? "border-amber-200 bg-amber-50 text-amber-800" : "border-emerald-200 bg-emerald-50 text-emerald-800"}`}>
        <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-sm font-semibold">{riskBanner.title}</p>
          <p className="text-xs opacity-80">{riskBanner.desc}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-[1.45fr_0.55fr]">
        <Card className="border-slate-200 bg-white">
          <CardHeader className="pb-0">
            ...
            <CardDescription className="text-xs">{store.name} 按日汇总</CardDescription>
          </CardHeader>
          <CardContent>
                ...
                <AreaChart data={trendData} margin={{ top: 12, right: 16, left: 0, bottom: 0 }}>
                  <defs>
                    <linearGradient id="storeRevenueFill" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%" stopColor={storeColor} stopOpacity={0.24} />
                      <stop offset="95%" stopColor={storeColor} stopOpacity={0.02} />
                    </linearGradient>
                    <linearGradient id="storeProfitFill" x1="0" y1="0" x2="0" y2="1">
                  ...
                  <Area type="monotone" dataKey="收入" stroke={storeColor} fill="url(#storeRevenueFill)" strokeWidth={2.5} />
                  <Area type="monotone" dataKey="利润" stroke="#059669" fill="url(#storeProfitFill)" strokeWidth={2.5} />
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
        <Card className="xl:col-span-2">
          <CardHeader className="pb-0">
            <CardTitle className="text-base">利润贡献 TOP10</CardTitle>
            <CardDescription className="text-xs">当前周期单品利润排行</CardDescription>
          </CardHeader>
          <CardContent>
            {profitRanking.length === 0 ? (
              <EmptyPanel text="暂无利润排行数据" />
            ) : (
              <ResponsiveContainer width="100%" height={270}>
                <BarChart data={profitRanking} margin={{ top: 8, right: 16, left: 0, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#eef2f7" vertical={false} />
                  <XAxis dataKey="sku" fontSize={11} tickLine={false} axisLine={false} />
                  <YAxis fontSize={11} tickLine={false} axisLine={false} tickFormatter={(value) => `$${Number(value).toFixed(0)}`} />
                  <Tooltip formatter={(value) => [formatUsd(Number(value)), "净利润"]} />
                  <Bar dataKey="value" fill={storeColor} radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            )}
          </CardContent>
        ...
        <Card>
          <CardHeader className="pb-0">
            ...
            <CardDescription className="text-xs">单店分配 SKU 的数量与货值</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
